...
def model_predict(data_test, y_label, learning_rate = 0.009, num_epochs = 100, minibatch_size = 10, print_cost = True):
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess, "D:\\Personal\\ITM\\Diplomado Deep Learning\\RedesConvolucionales_Clase1_empty\\model_softmax.ckpt")        
...
img = cv2.imread("D:\\Personal\\ITM\\Diplomado Deep Learning\\RedesConvolucionales_Clase1_empty\\figura2.png")
...
#Predecir una lista
#y_label= np.array([2])
#y_label = convert_to_one_hot(Y_train_orig, 6).T
##
#prediccion = model_predict(X_train_orig, y_label)

#predicción con mnist
y_label= np.array([2])
y_label = Y_test_orig.T
X_test_orig = X_test_orig[...:np.newaxis]/255
prediccion = model_predict(X_test_orig, y_label)
